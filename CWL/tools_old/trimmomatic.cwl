...
inputs:
  ...
  headcrop:
    doc: "<length>\nRemoves the specified number of bases, regardless of quality,\
      \ from the beginning of the read.\n<length>: The number of bases to keep, from\
...
- position: 8
  valueFrom: "${\n  if (inputs.end_mode == \"PE\" && inputs.input_read2_fastq_file)\n\
    \    return inputs.input_read1_fastq_file.path.replace(/^.*[\\\\\\/]/, '').replace(/\\\
    .[^/.]+$/, '') + '_trimmed_unpaired.fastq';\n  return null;\n}\n"
- position: 9
  valueFrom: "${\n  if (inputs.end_mode == \"PE\" && inputs.input_read2_fastq_file)\n\
    \    return inputs.input_read2_fastq_file.path.replace(/^.*[\\\\\\/]/, '').replace(/\\\
    .[^/.]+$/, '') + '_trimmed.fastq';\n  return null;\n}\n"
- position: 10
  valueFrom: "${\n  if (inputs.end_mode == \"PE\" && inputs.input_read2_fastq_file)\n\
    \    return inputs.input_read2_fastq_file.path.replace(/^.*[\\\\\\/]/, '').replace(/\\\
...
outputs:
  ...
  output_read1_trimmed_unpaired_file:
    type: File?
    outputBinding:
      glob: "${\n  if (inputs.end_mode == \"PE\")\n    return inputs.input_read1_fastq_file.path.replace(/^.*[\\\
